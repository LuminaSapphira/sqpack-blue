const CRC_INITIAL_SEED: u32 = 0xFFFFFFFF;
...
pub fn compute_with_seed(seed: u32, buffer: &[u8], start: u32, size: u32) -> u32 {
    let mut crc = seed;
    for i in start..size+start {
        ...
        crc = (crc >> 8) ^ CRC_TABLE[(b as u8 ^ crc as u8) as usize];
    }
    crc
}

pub fn compute(val: &String) -> u32 {
